...
while (have_posts()) : the_post();
    ...
    ?>
    <div class="it_epoll_container"<?php if ($it_epoll_poll_container_color_primary and $it_epoll_poll_container_color_secondary) { ?> style="background: 
            -webkit-linear-gradient(40deg,<?php echo $it_epoll_poll_container_color_secondary; ?>,<?php echo $it_epoll_poll_container_color_primary; ?>)!important;
        ...
        <div class="it_epoll_inner">
            <ul class="it_epoll_surveys <?php if ($it_epoll_poll_style == 'list') echo 'it_epoll_list'; else echo 'it_epoll_grid'; ?>">
                ...
                if ($it_epoll_option_names) {
                    $total_percentage = 0;
                    $last_index = count($it_epoll_option_names) - 1;
                    $percentages = [];
                    ...
                    foreach ($it_epoll_option_names as $it_epoll_option_name) {
                        ...
                        if ($it_epoll_poll_vote_count != 0) {
                            $percentage = $it_epoll_poll_vote_count * 100 / $it_epoll_poll_vote_total_count;
                        }
                    ...
                    // Adjust the last option to ensure the total is 100%
                    if ($total_percentage != 100) {
                        $percentages[$last_index] += (100 - $total_percentage);
                    }

                    $i = 0;
                    foreach ($it_epoll_option_names as $it_epoll_option_name) {
                        $it_epoll_poll_vote_percentage = round($percentages[$i]);

                        if ($it_epoll_poll_style == 'list') { ?>
